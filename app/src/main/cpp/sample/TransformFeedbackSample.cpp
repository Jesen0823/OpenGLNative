...
void TransformFeedbackSample::Init() {
    ...
    // 生成 VBO ，加载顶点数据和索引数据,Generate VBO Ids and load the VBOs with data
    ...
    glBufferData(GL_TRANSFORM_FEEDBACK_BUFFER, (3 + 2) * 6 * sizeof(GLfloat), nullptr, GL_STATIC_READ);
...
void TransformFeedbackSample::Draw(int screenW, int screenH) {
    ...
    glBindBuffer(GL_TRANSFORM_FEEDBACK_BUFFER, m_TransFeedbackBufId);
    void *rawData = glMapBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 0, (3 + 2) * 6 * sizeof(GLfloat),
                                     GL_MAP_READ_BIT);

    float *p = (float *) rawData;
    for (int i = 0; i < 6; i++) {
        LOGCATE("TransformFeedbackSample::Draw() read feedback buffer outPos[%d] = [%f, %f, %f], outTex[%d] = [%f, %f]",
