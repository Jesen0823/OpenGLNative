...
void RGB2I420Sample::Init() {
    ...
    char fFboShaderStr[] =
            ...
            "        }else {                                                                    \n"
            "            texCoord = vec2((v_texCoord.x - 0.5) * 2.0, "
            "                       ((v_texCoord.y - U_DIVIDE_LINE) * 2.0 + offsetY) * 3.0);    \n"
            "        }                                                                          \n"
            "                                                                                   \n"
            "        vec4 color0 = texture(s_TextureMap, texCoord);                             \n"
            ...
            "        }else {                                                                    \n"
            "            texCoord = vec2((v_texCoord.x - 0.5) * 2.0, "
            "                       ((v_texCoord.y - V_DIVIDE_LINE) * 2.0 + offsetY) * 3.0);    \n"
            "        }                                                                          \n"
            "                                                                                   \n"
            "        vec4 color0 = texture(s_TextureMap, texCoord);                             \n"
            "        vec4 color1 = texture(s_TextureMap, texCoord + texelOffset * 2.0);         \n"
...
// 离屏渲染
    ...
    nativeImage.ppPlane[2] =
            nativeImage.ppPlane[1] + m_RenderImage.width * m_RenderImage.height / 4;
    BEGIN_TIME("FBO glReadPixels")
        glReadPixels(0, 0, nativeImage.width / 4, nativeImage.height * 1.5, GL_RGBA,
...
bool RGB2I420Sample::CreateFrameBufferObj() {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_RenderImage.width / 4,
                 m_RenderImage.height * 1.5, 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 nullptr);
