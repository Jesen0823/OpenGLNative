...
void PortraitStayColorExample::Init() {
    ...
    glBindTexture(GL_TEXTURE_2D, m_GrayTexId);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_LUMINANCE, m_GrayImage.width, m_GrayImage.height, 0,
                 GL_LUMINANCE, GL_UNSIGNED_BYTE, m_GrayImage.ppPlane[0]);
    glBindTexture(GL_TEXTURE_2D, GL_NONE);

    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_2D, m_MappingTexId);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_MappingImage.width, m_MappingImage.height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, m_MappingImage.ppPlane[0]);
    glBindTexture(GL_TEXTURE_2D, GL_NONE);
}
...
void PortraitStayColorExample::LoadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGCATE("PortraitStayColorExample::LoadMultiImageWithIndex pImage = %p,[w=%d,h=%d,f=%d]",
...
void PortraitStayColorExample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY,
                                               float ratio) {
    LOGCATE("PortraitStayColorExample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f",
            angleX, angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
