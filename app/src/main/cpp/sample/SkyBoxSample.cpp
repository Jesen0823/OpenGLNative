...
void SkyBoxSample::LoadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGCATE("SkyBoxSample::LoadMultiImageWithIndex pImage = %p, index=%d", pImage->ppPlane[0],
            index);
    if (pImage) {
...
void SkyBoxSample::Draw(int width, int height) {
    LOGCATE("SkyBoxSample::Draw()");
    if (m_ProgramObj == GL_NONE) return;
    glEnable(GL_DEPTH_TEST);

    UpdateMVPMatrix(m_MVPMatrix, m_AngleX, m_AngleY, 1.0, (float) width / height);
    ...
    UpdateMVPMatrix(m_MVPMatrix, m_AngleX, m_AngleY, 0.4f, (float) width / height);
