...
void GLTransitionExample_4::Init() {
    ...
    char fShaderStr[] =
            ...
            "                                                                                  \n"
            "// How much the bars seem to \"run\" from the middle of the screen first "
            "(sticking to the sides). 0 = no drip, 1 = curved drip                             \n"
            "const float dripScale = 0.5;                                                      \n"
            "                                                                                  \n"
            "// The code proper --------                                                       \n"
            "float rand(int num) {                                                             \n"
            "    return fract(mod(float(num) * 67123.313, 12.0) * sin(float(num) * 10.3) "
            "           * cos(float(num)));                                                    \n"
            "}                                                                                 \n"
            "                                                                                  \n"
            "float wave(int num) {                                                             \n"
...
void
GLTransitionExample_4::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGCATE("GLTransitionExample_4::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX,
            angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
...
void GLTransitionExample_4::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX,
                                                  float scaleY) {
...
void GLTransitionExample_4::LoadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGCATE("GLTransitionExample_4::LoadMultiImageWithIndex pImage = %p,[w=%d,h=%d,f=%d]",
