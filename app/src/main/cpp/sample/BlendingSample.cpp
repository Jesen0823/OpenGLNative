...
void BlendingSample::Init() {
    ...
    for (GLuint i = 0; i < windowsTrans.size(); i++) {
        GLfloat distance = std::sqrt(std::pow(windowsTrans[i].x - 0.5f, 2.0f) +
                                     std::pow(windowsTrans[i].y - 1.0f, 2.0f) +
                                     std::pow(windowsTrans[i].z - 3.0f, 2.0f));
...
void
BlendingSample::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX,
                                      float scaleY) {
...
void
BlendingSample::UpdateMatrix(glm::mat4 &mvpMatrix, int angleXRotate, int angleYRotate,
                             float scale,
