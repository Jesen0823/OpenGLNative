...
void FaceSlenderSample::Init() {
    ...
    char fShaderStr[] =
            ...
            "                                                                                   \n"
            "vec2 face_slender_1(vec2 prePoint, vec2 curPoint, "
            "               vec2 texCoord, float radius, vec2 texSize){                         \n"
            "    vec2 pos = texCoord;                                                           \n"
            "                                                                                   \n"
            ...
            "}                                                                                  \n"
            "                                                                                   \n"
            "vec2 face_slender_2(vec2 prePoint, vec2 curPoint, vec2 texCoord, float radius, "
            "                    vec2 texSize){                                                 \n"
            "    vec2 pos = texCoord;                                                           \n"
            "                                                                                   \n"
            "    vec2 newSrc = prePoint * texSize;                                              \n"
            ...
            "                                                                                   \n"
            "    if(r < newRadius){                                                             \n"
            "        float gamma = (pow(newRadius, 2.0) - pow(r, 2.0)) / (pow(newRadius, 2.0) - pow(r, 2.0) "
            "                     + pow(distance(newDst, newSrc), 2.0));                        \n"
            ...
            "                                                                                   \n"
            "    vec2 newTexCoord = face_slender_1(leftPreCtrl, leftCurCtrl, v_texCoord, "
            "                       u_reshapeRadius, u_texSize);                                \n"
            "    newTexCoord = face_slender_1(rightPreCtrl, rightCurCtrl, newTexCoord, "
            "                  u_reshapeRadius, u_texSize);                                     \n"
            "                                                                                   \n"
...
void FaceSlenderSample::Draw(int screenW, int screenH) {
    ...
    float effectRadius = PointUtil::Distance(PointF(LeftCheekKeyPoint[0], LeftCheekKeyPoint[1]),
                                             PointF(ChinKeyPoint[0], ChinKeyPoint[1])) / 2;
    ...
    GLUtils::setVec4(m_ProgramObj, "u_preCtrlPoints",
                     LeftSlenderCtlPoint[0] / m_RenderImage.width,
                     LeftSlenderCtlPoint[1] / m_RenderImage.height,
                     RightSlenderCtlPoint[0] / m_RenderImage.width,
                     RightSlenderCtlPoint[1] / m_RenderImage.height);

    PointF leftCurPoint = PointUtil::PointAdd(PointF(LeftCheekKeyPoint[0], LeftCheekKeyPoint[1]),
                                              PointF(ChinKeyPoint[0], ChinKeyPoint[1]));
    leftCurPoint = PointUtil::PointDivide(leftCurPoint, 2);

    PointF rightCurPoint = PointUtil::PointAdd(PointF(RightCheekPoint[0], RightCheekPoint[1]),
                                               PointF(ChinKeyPoint[0], ChinKeyPoint[1]));
...
void FaceSlenderSample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGCATE("FaceSlenderSample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX,
            angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
