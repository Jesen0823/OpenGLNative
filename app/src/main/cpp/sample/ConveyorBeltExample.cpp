...
void ConveyorBeltExample::Init() {
    ...
    glm::vec2 pTmpCoords[BF_TEXTURE_NUM * 2 * 3];
    //第一块矩形
    pTmpCoords[0] = glm::vec2(0, 0);
    pTmpCoords[1] = glm::vec2(0, 0.5);
    pTmpCoords[2] = glm::vec2(1, 0.5);

    pTmpCoords[3] = glm::vec2(0, 0);
    pTmpCoords[4] = glm::vec2(1, 0.5);
    pTmpCoords[5] = glm::vec2(1, 0);

    m_pTexCoords[0] = glm::vec2(0, 0);
    m_pTexCoords[1] = glm::vec2(0, 1);
    m_pTexCoords[2] = glm::vec2(1, 1);

    m_pTexCoords[3] = glm::vec2(0, 0);
    m_pTexCoords[4] = glm::vec2(1, 1);
    m_pTexCoords[5] = glm::vec2(1, 0);

    float delta = 0.5f / BF_BANNER_NUM;
    //其他小矩形
    for (int i = 1; i <= BF_BANNER_NUM; ++i) {
        glm::vec2 p0 = glm::vec2(0, 0.5 + (i - 1) * delta);
        glm::vec2 p1 = glm::vec2(0, 0.5 + i * delta);
        glm::vec2 p2 = glm::vec2(1, 0.5 + i * delta);
        glm::vec2 p3 = glm::vec2(1, 0.5 + (i - 1) * delta);
        ...
        m_pTexCoords[6 * i] = glm::vec2(0, 0);
        m_pTexCoords[6 * i + 1] = glm::vec2(0, 1);
        m_pTexCoords[6 * i + 2] = glm::vec2(1, 1);

        m_pTexCoords[6 * i + 3] = glm::vec2(0, 0);
        m_pTexCoords[6 * i + 4] = glm::vec2(1, 1);
        m_pTexCoords[6 * i + 5] = glm::vec2(1, 0);
    }
    ...
    memcpy(m_TexImages[0].ppPlane[0], m_RenderImage.ppPlane[0],
           m_TexImages[0].width * m_TexImages[0].height * 4);

    glm::uint8 *pBuf = m_RenderImage.ppPlane[0] + m_TexImages[0].width * m_TexImages[0].height * 4;
    for (int i = 1; i <= BF_BANNER_NUM; ++i) {
    ...
    for (int i = 0; i < BF_TEXTURE_NUM; ++i) {
        ...
        glBindTexture(GL_TEXTURE_2D, m_TextureIds[i]);
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, m_TexImages[i].width, m_TexImages[i].height, 0,
                     GL_RGBA, GL_UNSIGNED_BYTE, m_TexImages[i].ppPlane[0]);
        glBindTexture(GL_TEXTURE_2D, GL_NONE);
    }
}

void ConveyorBeltExample::LoadImage(NativeImage *pImage) {
    LOGCATE("ConveyorBeltExample::LoadImage pImage = %p,[w=%d,h=%d,f=%d]", pImage->ppPlane[0],
            pImage->width, pImage->height, pImage->format);
    if (pImage) {
...
void ConveyorBeltExample::Draw(int screenW, int screenH) {
    ...
    glm::uint8 *pBuf = m_RenderImage.ppPlane[0] + m_TexImages[0].width * m_TexImages[0].height * 4 -
                       m_TexImages[1].width * m_TexImages[1].height * 4;
    for (int i = BF_BANNER_NUM; i >= 2; --i) {
        memcpy(m_TexImages[i].ppPlane[0], m_TexImages[i - 1].ppPlane[0],
               m_TexImages[i].width * m_TexImages[i].height * 4);
    }
...
void
ConveyorBeltExample::UpdateMVPMatrix(glm::mat4 &mvpMatrix, int angleX, int angleY, float ratio) {
    LOGCATE("ConveyorBeltExample::UpdateMVPMatrix angleX = %d, angleY = %d, ratio = %f", angleX,
            angleY, ratio);
    angleX = angleX % 360;
    angleY = angleY % 360;
...
void ConveyorBeltExample::UpdateTransformMatrix(float rotateX, float rotateY, float scaleX,
                                                float scaleY) {
...
void ConveyorBeltExample::LoadMultiImageWithIndex(int index, NativeImage *pImage) {
    LOGCATE("ConveyorBeltExample::LoadMultiImageWithIndex pImage = %p,[w=%d,h=%d,f=%d]",
